...
class WordSegmentation(object):
    ...
    def segment_sentence(self, sentence, lower = True, with_stop_words = True, speech_tag_filter=False):
        '''
        将列表sequences中的每个句子转换为由单词构成的列表。
        ...
        res.append(self.segment(text=sentence, 
                                    ...
                                    speech_tag_filter=speech_tag_filter))
        return res
    def segment_sentences_doc_weight(self, text, lower = True, with_stop_words = True, speech_tag_filter=False):
        doc = text
        res = []
        weight = []
        sentences = []
        for paragraph in doc['paragraphs']:
                for sentence in paragraph['sentences']:
                    res.append(self.segment(text=sentence['content'], 
                                    lower=lower, 
                                    with_stop_words=with_stop_words, 
                                    speech_tag_filter=speech_tag_filter))
                    weight.append(sentence['weight'])
                    sentences.append(sentence['content'])
        return sentences,res,weight
...
class Segmentation(object):
    ...
    def segment_sentences_weight(self,text):
        (sentences,words_all_filters,weight) = self.ws.segment_sentences_doc_weight(text=text, lower = False, with_stop_words = True,speech_tag_filter = True)
        return sentences,words_all_filters,weight

    def segment_sentence(self,sentence):
        words_all_filters = self.ws.segment_sentence(sentence=sentence, 
                                                    lower = False, 
                                                    with_stop_words = True,
                                                    speech_tag_filter = True)

        return words_all_filters
